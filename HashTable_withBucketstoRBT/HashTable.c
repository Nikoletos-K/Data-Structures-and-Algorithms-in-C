...
unsigned int hashFunction(char * str,unsigned int length){		// hash function that uses prime numbers in order to eliminate collitions
...
int insert_toHashTable(void * value,char * HTkey,char * treeKey,HashTable * ht,int (*comparator)(comparatorValue,comparatorValue)){
	...
	unsigned int hash = hashFunction(HTkey,strlen(HTkey)+1)%(ht->hashtableSize);
...
int findKeyData(HashTable * hashtable,char * wantedKey,char * date1,char * date2,int (*comparator)(comparatorValue,comparatorValue),char * funValue,int (*function)(void*,char*)){
	...
	int loops = 0;
	int hash = hashFunction(wantedKey,strlen(wantedKey)+1)%(hashtable->hashtableSize);
	...
	while(tempBucket!=NULL){	/* search the list of buckets */
		...
		while(loops<tempBucket->records){	/* search inside buckets */
			...
			if(strcmp(wantedKey,tempRecord->key)==0){		/* find specific hashtable key */
				int counter = 0;
				RBTNode * treeRoot = (RBTNode *) getValue(tempRecord);
...
int printConditionHT(HashTable * hashtable,int (*condition)(void*,char*)){  // searches all the hashtable and prnts for every disease the number of patients

	int loops=0,counter=0,totalCounter=0;
	for(int i=0;i<hashtable->hashtableSize;i++){
		...
		while(tempBucket!=NULL){	/* search the list of buckets */
			...
			while(loops<tempBucket->records){	/* search inside buckets */
				...
				counter=0;
				RBTNode * treeRoot = (RBTNode *) getValue(tempRecord);
				RBTFindNodesBetweenKeys(treeRoot,&counter,NULL,NULL,tempRecord->key,comparator,condition);
				#ifdef WITH_UI
					printf("\t- %s: %d \n",tempRecord->key,counter);
				#else
					printf("%s %d\n",tempRecord->key,counter);
